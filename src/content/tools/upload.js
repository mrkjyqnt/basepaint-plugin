...
(function () {
  ...
  BP.processUploadedImage = async function (file, palette, opts) {
    ...
    for (let y = 0; y < BP.CANVAS_SIZE; y++) {
      for (let x = 0; x < BP.CANVAS_SIZE; x++) {
        ...
        const colorIndex = exact !== -1 ? exact : BP.getNearestColorIndex(r, g, b, lookup);
        if (currentCanvas) {
          const key = x + ',' + y;
  ...
  BP.findBasePaintComponent = function () {
    ...
    if (pasteBtn) sources.push(pasteBtn);
    const copyBtn = Array.from(document.querySelectorAll('button')).find((b) => b.title === 'Copy');
    if (copyBtn) sources.push(copyBtn);
    const saveBtn = document.querySelector('button#commit') ||
      Array.from(document.querySelectorAll('button')).find((b) => /save|commit/i.test(b.title || b.id || ''));
    if (saveBtn) sources.push(saveBtn);
    document.querySelectorAll('#toolbar *').forEach((el) => sources.push(el));
      ...
      while (f && depth++ < 50) {
        ...
        if (t && typeof t !== 'string' && f.memoizedProps) {
          ...
          if (typeof p.onPaste === 'function') {
            best = f;
            bestSrc = el.tagName + (el.id ? '#' + el.id : '') + (el.title ? '[' + el.title + ']' : '');
            break;
          }
        }
        f = f.return;
    ...
    if (best) {
      console.log('[basepaint-plugin] findBasePaintComponent: found F via ' + bestSrc);
    } else {
      console.warn('[basepaint-plugin] findBasePaintComponent: no F found (searched ' + sources.length + ' starting points)');
    }
    return best;
  };
  ...
  BP.showUploadPreview = function (img, pixels, palette, opts) {
    return new Promise((resolve) => {
      ...
      desc.textContent = "Pick a file. Pixels are matched to today's palette and applied as basepaint strokes.";
      ...
      function renderPreview(pixList, keepExisting) {
        ...
        const rangeText = r ? `Range: x[${r.minX}–${r.maxX}] y[${r.minY}–${r.maxY}]` : 'Range: —';
        if (keepExisting) {
      ...
      cb.addEventListener('change', (e) => {
        ...
        if (e.target.checked && currentCanvasMap && currentCanvasMap.size > 0) {
          const filtered = fullPixels.filter((p) => currentCanvasMap.get(p.point.x + ',' + p.point.y) !== p.color);
      ...
      noOverprintLabel.textContent = 'No overprint — skip pixels that already match the canvas';
      ...
      hint.textContent = 'When "no overprint" is on, the preview shows only pixels that change.';
      ...
      confirmBtn.className = 'basepaint-plugin-modal-btn basepaint-plugin-modal-btn--primary';
      confirmBtn.textContent = '✓  Paste to canvas';
      confirmBtn.addEventListener('click', () => {
        const noOverprint = noOverprintRef.checked;
        const finalPixels = noOverprint && currentCanvasMap && currentCanvasMap.size > 0
          ? fullPixels.filter((p) => currentCanvasMap.get(p.point.x + ',' + p.point.y) !== p.color)
          : fullPixels;
        overlay.remove();
  ...
  BP.executeImageUpload = async function (canvas) {
    ...
    const img = await BP.loadImage(file);
    const currentCanvasMap = BP.readCanvasAsColorMap ? BP.readCanvasAsColorMap(canvas, palette) : null;
    ...
    if (allPixels.length === 0) {
      ...
      alert('No pixels in this image match today\'s palette colors.\nMake sure your image uses the exact palette colors.');
      return 0;
    }

    const confirmed = await BP.showUploadPreview(img, allPixels, palette, { currentCanvas: currentCanvasMap });
    ...
    console.log(`[basepaint-plugin] Rendered ${pixels.length} pixels to PNG canvas`);
    ...
    console.log(`[basepaint-plugin] Stroke coord range: x[${minX}-${maxX}] y[${minY}-${maxY}] (should be 0-255)`);

    const result = await BP.applyStrokesDirectly(pixels);
    console.log(`[basepaint-plugin] Apply strokes: ${JSON.stringify(result)}`);

    if (result.ok) {
      console.log(`[basepaint-plugin] ✅ ${pixels.length} strokes applied to basepaint canvas`);
      return pixels.length;
    }

    writePixelsToCanvasDirect(canvas, pixels, palette);
    console.warn(`[basepaint-plugin] Auto-paste failed: ${result.reason}; image drawn as preview pixels only`);
    return pixels.length;
  };
