...
(function () {
  ...
  BP.exportCanvasPNG = function (canvas, filenameOrOpts) {
    ...
    if (Array.isArray(opts.strokes) && opts.strokes.length) {
      const palette = opts.palette || BP.readPaletteFromDOM() || [];
      for (const s of opts.strokes) {
        const hex = palette[s.color];
        if (!hex) continue;
        ctx.fillStyle = hex;
        ctx.fillRect(s.point.x, s.point.y, 1, 1);
      }
    } else {
