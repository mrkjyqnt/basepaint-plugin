(function () {
  ...
  function readMyStrokesFromClipboard() {
    const copyBtn =
      document.querySelector('#toolbar button[title="Copy Strokes"]') ||
    ...
    try {
      ...
      console.warn(
        "[basepaint-plugin] Could not parse clipboard strokes:",
        e.message,
      );
    }
  ...
  async function diffStrokesAgainstApi() {
    ...
    for (let ly = 0; ly < SIZE; ly++) {
      for (let lx = 0; lx < SIZE; lx++) {
        ...
        const ur = userData[uIdx],
          ug = userData[uIdx + 1],
          ub = userData[uIdx + 2];
        const ar = apiData[aIdx],
          ag = apiData[aIdx + 1],
          ab = apiData[aIdx + 2];
        if (ur !== ar || ug !== ag || ub !== ab) {
          const exact = BP.getColorIndex(ur, ug, ub, lookup);
          const color =
            exact !== -1 ? exact : BP.getNearestColorIndex(ur, ug, ub, lookup);
  ...
  function openDownloadModal() {
    ...
    desc.textContent =
      "Pick a format. PNG is just your strokes on a transparent background; text files are YOUR strokes in the basepaint toolbox paste format.";
    ...
    const mkBtn = (label, onClick, primary) => {
      ...
      b.className =
        "basepaint-plugin-modal-btn" +
        (primary ? " basepaint-plugin-modal-btn--primary" : "");
      b.textContent = label;
    ...
    btnStack.appendChild(
      mkBtn(
        "⬇  Download as PNG (256×256)",
        () => {
    ...
    btnStack.appendChild(
      mkBtn("Download strokes (single text file)", () => {
    ...
    splitInput.className =
      "basepaint-plugin-modal-input basepaint-plugin-modal-input--narrow";
    ...
    splitNote.textContent =
      "Each chunk is one paste-ready text file you can paste into the basepaint toolbox, one at a time.";
  ...
  async function downloadStrokesSingle() {
    ...
    if (!strokes.length) {
      console.warn(
        "[basepaint-plugin] No strokes to download — draw some on the canvas first.",
      );
      return;
    }
  ...
  async function downloadStrokesSplit(chunkSize) {
    ...
    if (!strokes.length) {
      console.warn(
        "[basepaint-plugin] No strokes to split — draw some on the canvas first.",
      );
      return;
    }
    const sections = [];
    ...
    console.log(
      `[basepaint-plugin] Split ${strokes.length} strokes into ${total} files`,
    );
  }
