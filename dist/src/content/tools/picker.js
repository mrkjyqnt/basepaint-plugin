...
(function () {
  ...
  BP.executeColorPick = function (canvas, logicalX, logicalY, palette) {
    ...
    console.log(`[basepaint-plugin] Picked raw: ${raw} (a=${pixel.a}) at logical (${logicalX}, ${logicalY})`);

    const swatchPalette = palette && palette.length ? palette : BP.readPaletteFromSwatches();
    ...
    const nearestIdx = BP.getNearestColorIndex(pixel.r, pixel.g, pixel.b, lookup);
    const target = swatchPalette[nearestIdx];
    console.log(`[basepaint-plugin] Picked color ${raw} → nearest palette color ${target} (idx ${nearestIdx})`);
    ...
    if (typeof BP.findBasePaintComponent === 'function') {
      ...
      if (F) {
        ...
        if (F.memoizedProps?.drawMode === false && typeof F.memoizedProps.onToggleDrawMode === 'function') {
          try {
        ...
        if (F.memoizedProps?.eraseMode === true && typeof F.memoizedProps.onToggleErase === 'function') {
          try {
